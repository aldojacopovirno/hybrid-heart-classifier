...
class DataAnalyzer:
    ...
    def plot_distributions(self):
        fig, axes = plt.subplots(nrows=2, ncols=8, figsize=(40, 10))
        axes = axes.flatten()
 
        for i, var in enumerate(self.numerical_vars):
            if i < 16:  # Limit to 16 plots
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.plot_folder, 'all_distributions.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_correlation_heatmap(self):
        ...
        plt.savefig(os.path.join(self.plot_folder, 'correlation_heatmap.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_pairplot(self):
        ...
        plt.savefig(os.path.join(self.plot_folder, 'pairplot.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_outliers(self, outliers):
        ...
        plt.legend()
        plt.savefig(os.path.join(self.plot_folder, 'outliers_pca.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_kmeans_clusters(self, df_scaled, pca):
        ...
        plt.xlabel('First Principal Component', fontsize=12)
        plt.ylabel('Second Principal Component', fontsize=12)
        plt.savefig(os.path.join(self.plot_folder, 'kmeans_clusters.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_categorical_vs_numerical(self):
        for cat_var in self.categorical_vars:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.plot_folder, f'variables_vs_{cat_var}.png'), dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def plot_qqplots(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.plot_folder, 'all_qqplots.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_cumulative_variance_ratio(self, pca):
        ...
        plt.savefig(os.path.join(self.plot_folder, 'cumulative_variance_ratio.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_analysis(self):
        ...
        print(f"\nGenerating visualizations in folder: {self.plot_folder}")
        ...
        print(f"\nAdvanced Comprehensive EDA completed. Please review the generated visualizations in the '{self.plot_folder}' folder and the statistical outputs above.")
