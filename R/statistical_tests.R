...
#'   \item{Descriptive Statistics}{Basic statistics including mean, median, etc.}
#'   \item{Confidence Intervals and P-value}{Confidence intervals and t-test results}
#'   \item{Outlier Analysis}{Outlier counts using IQR method}
#'   \item{Correlation Matrix}{Pearson correlation matrix}
#' @details
#' Calculates:
...
statistical_analysis <- function(df) {
  ...
  basic_stats <- data.frame(
    ...
    Minimum = apply(df_numeric, 2, min, na.rm = TRUE),
    ...
    Maximum = apply(df_numeric, 2, max, na.rm = TRUE),
  ...
  outliers_table <- data.frame(
    Variable = names(df_numeric),
    Number_of_Outliers = outliers_count
  )
  ...
  output <- list(
    ...
    "Confidence Intervals and P-value" = ci_pvalue_table,
    "Outlier Analysis" = outliers_table,
    "Correlation Matrix" = formatted_correlation_matrix
  )
...
#' @return A list containing:
#'   \item{Normality Test Results}{Results of multiple normality tests}
#'   \item{Test Power}{Power analysis results}
#'   \item{Corrected Significance Level}{Bonferroni-corrected significance level}
#' @details
#' Performs:
#' - Shapiro-Wilk test
...
normality_tests_selected <- function(df, selected_vars = c("age", "trestbps", "chol", "thalch"), 
  ...
  output <- list(
    "Normality Test Results" = results[, 1:5],
    "Test Power" = results[, c("Variable", "SW_Power", "KS_Power", "JB_Power", "AD_Power")],
