...
statistical_analysis <- function(df) {
  ...
  mins <- apply(df_numeric, 2, min, na.rm = TRUE)                          
  quantile1st <- apply(df_numeric, 2, function(x) quantile(x, probs = 0.25, na.rm = TRUE))  
  medians <- apply(df_numeric, 2, median, na.rm = TRUE)                      
  means <- colMeans(df_numeric, na.rm = TRUE)                                
  quantile3rd <- apply(df_numeric, 2, function(x) quantile(x, probs = 0.75, na.rm = TRUE))
  maxes <- apply(df_numeric, 2, max, na.rm = TRUE)                            
  vars <- apply(df_numeric, 2, var, na.rm = TRUE)                             
  ...
  return(analysis_results)  # Restituisce il dataframe dei risultati
}
